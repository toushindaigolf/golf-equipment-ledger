...
function Terms({ contactConfigured }: { contactConfigured: boolean }) {
  return <>
    ...
    <p>重要なデータは、保存したデータファイルを利用者自身でも定期的に保管してください。移行操作の前にもデータファイルの保存をおすすめします。</p>
    <h3>4. Free版とPro版</h3>
...
function Help() {
  return <>
    <p className="public-info-lead">最初に保存したデータファイルの場所を確認しておくと、安心して記録を続けられます。</p>
    <ol className="public-info-steps">
      ...
      <li><strong>データを保存する</strong><span>「データを保存」で全用品をファイルとして保存します。「保存データを読み込む」は現在の端末内記録を置き換えるため、実行前に内容を確認してください。</span></li>
      ...
      <li><strong>移行前にもデータを保存する</strong><span>移行機能は開始前に保存データファイルを作成しますが、大切なデータは利用者自身でも保管してください。</span></li>
    </ol>
  </>;
}
